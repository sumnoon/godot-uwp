...
>
> **Canonical implementations to copy from (this repository):**
> - Engine: the repository root — this Godot fork (4.8.0-dev) with the embedding patches
> - UWP app: `platform/windows/uwp_sample/GodotUWPSample` (namespace `Godot.Uwp.Embedding`)
> - Bundled Godot project: `platform/windows/uwp_sample/GodotUWPSample/GodotProject` (spinning-cube smoke test)
...
Apply to a clean engine checkout. These patches were developed on 4.6.4 and ported
to 4.8 (this fork); the only churn across versions is the `DisplayServer` API — 4.8
moved its enums into the `DisplayServerEnums` namespace and changed a few `window_*`
virtual signatures (mirror `DisplayServerHeadless` for the current shapes).
Everything is in `platform/windows` plus two D3D12 driver files — **no core engine
changes**.
